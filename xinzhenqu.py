...
#TODO
# 高德上申请的key
key = 'd3f18c1080e955a88d204bb1c41084aa'
#搜素的城市名(全名)
#addr_name = '南京市鼓楼区'
keywords="320106"
...
def getlnglat(keywords):

    uri = url + 'keywords=' +keywords + '&key=' + key + '&subdistrict=1' + '&extensions=all'

    # 访问链接后，api会回传给一个json格式的数据
    temp = urllib.request.urlopen(uri)
    temp = json.loads(temp.read().decode('utf-8'))
    print(temp)
...
if __name__ == '__main__':
    num = 0
    #ad = getSubName(keywords)  # 得到福州下属区域的城市代码

    add = getlnglat(keywords)  # 得到福州整个的边界数据
    print(add)
    # while num < len(ad):
    #     add = pd.concat([add, getlnglat(ad[num].encode("utf-8"))])  # 得到福州下属的全部区域的边界数据
    #     num += 1
    add.to_csv('F:\data\{}.csv'.format(keywords), encoding='gbk')